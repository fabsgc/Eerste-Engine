...
namespace ee
{
	...
	{
		EE_ASSERT_ERROR_SHORT(IsAffine());
	...
	void Matrix4::MakeProjectionOrtho(float left, float right, float top, float bottom, float _near, float _far)
	{
	...
	Matrix4 Matrix4::ProjectionPerspective(const Degree& horzFOV, float aspect, float _near, float _far, bool positiveZ)
	{
		...
		float half_w = tanThetaX * _near;
		float half_h = tanThetaY * _near;
		...
		float inv_d = 1 / (_far - _near);

		float A = 2 * _near * inv_w;
		float B = 2 * _near * inv_h;
		...
		if (_far == 0)
		{
			...
			qn = _near * (INFINITE_FAR_PLANE_ADJUST - 2);
		}
		else
		{
			q = sign * (_far + _near) * inv_d;
			qn = -2.0f * (_far * _near) * inv_d;
		}
	...
	Matrix4 Matrix4::ProjectionOrthographic(float left, float right, float top, float bottom, float _near, float _far)
	{
		Matrix4 output;
		output.MakeProjectionOrtho(left, right, top, bottom, _near, _far);
